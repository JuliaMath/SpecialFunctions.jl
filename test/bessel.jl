...
for i = 1:size(table10p11,1)
    ...
    bi  = table10p11[i,4]
    bip = table10p11[i,5]
    @test_approx_eq_eps ai SF.airyai(x) tol
    @test_approx_eq_eps aip SF.airyaiprime(x) tol
    @test_approx_eq_eps bi SF.airybi(x) tol
    @test_approx_eq_eps bip SF.airybiprime(x) tol
    x   = table10p11[i,6]
    ai  = table10p11[i,7]
    aip = table10p11[i,8]
    bi  = table10p11[i,9]
