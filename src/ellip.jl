...
# Core implementation of ellipe for m >= 0, using binary tree branching
# for O(log n) performance instead of O(n) linear if-elseif chain
@inline function _ellipe_core(m::Float64)
    ...
    # Integer index for binary tree branching (faster icmp vs fcmp)
    idx = unsafe_trunc(Int, m * 10)

    # Binary tree branching structure for O(log n) comparisons
    if idx < 5
        if idx < 2
            if idx == 0  # 0.0 < m < 0.1, Table 2
                t = m - 0.05
