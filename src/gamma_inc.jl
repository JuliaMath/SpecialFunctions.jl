...
const alog10 = log(10)
const rt2pin = Float64(IrrationalConstants.invsqrt2π)
const rtpi = Float64(IrrationalConstants.sqrtπ)
...
function stirling_error(x::Float64)
    ...
    elseif x < 1
        return loggamma1p(x) - (x + 0.5)*log(x) + x - IrrationalConstants.log2π/2.0
    elseif x < 2
        return loggamma1p(x - 1) - (x - 0.5)*log(x) + x - IrrationalConstants.log2π/2.0
    elseif x < 3
        return loggamma1p(x - 2) - (x - 0.5)*log(x) + x  - IrrationalConstants.log2π/2.0 + log(x - 1)
    elseif x < 12
        z=18.0/(x*x)-1.0
...
function gammax(x::Float64)
    ...
    elseif x > 0
        return gamma(x)/(exp(-x + (x - 0.5)*log(x))*IrrationalConstants.sqrt2π)
    else
        return floatmax(Float64)/1000.0
    end
end
...
function gamma_inc_inv_qsmall(a::Float64, q::Float64)
    b = 1.0 - a
    eta = sqrt(-2/a*log(q*gammax(a)*sqrt(IrrationalConstants.twoπ/a)))
    x0 = a*lambdaeta(eta)
    l = log(x0)
...
function gamma_inc_inv_alarge(a::Float64, porq::Float64, s::Integer)
    ...
    x0 = a*lambdaeta(eta)
    fp = -sqrt(IrrationalConstants.inv2π*a)*exp(-0.5*a*eta*eta)/gammax(a)
    return (x0, fp)
end
